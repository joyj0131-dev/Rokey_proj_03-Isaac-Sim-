...
- 인터페이스는 **팀원 관제(task_dispatcher)가 정의한 서비스 계약을 이름·타입
  그대로 사용**한다 (ROS 2 서비스, 관제=서버 / 로봇=클라이언트):
  - `acquire_zones` (`parking_robot_interfaces/srv/AcquireZones`):
    요청 `robot_id, zone_ids[]`(오름차순 필수) → 응답 `granted, held_zones[], retry_after_sec`
  - `release_zones` (`ReleaseZones`): 요청 `robot_id, zone_ids[]`(빈 배열 = 전부 반납)
    → 응답 `success`
  - 관제 쪽 규약 (로봇이 지켜야 할 계약): ① zone_ids 미정렬 요청은 무조건 거절,
    ② 다중 존 요청은 전부-아니면-무(하나라도 실패 시 롤백 후 거절).
- 이 노드는 팀원 관제가 부재한 동안 같은 서비스를 제공하는 **대역(stand-in)**이다
  (`sim_orchestrator`가 진짜 로봇의 대역인 것과 동일한 팀 패턴). 팀원 관제가 뜨면
  이 노드만 내리면 로봇 쪽(②③)은 변경 없이 연결된다.
- 장부는 `zone_ledger.py`(순수 파이썬 dict) — 불변식: 존당 점유자 최대 1,
  release는 소유자만 가능(아니면 무시+경고). all-or-nothing 롤백 포함해
  관제의 db 모드와 동일 시맨틱.
- **선결 항목**: `parking_robot_interfaces` 패키지(.srv 원본)가 어느 브랜치에도
  없음 — 관제 담당 팀원에게 확보. 그 전까지는 위 필드의 동형 srv를 임시 정의.
...
- 존 요청 단위는 **구간(leg) 일괄 획득** — 관제 코드가 상정한 호출 규약(다중 존을
  오름차순 배열로 한 번에, 전부-아니면-무, 거절 시 `retry_after_sec` 후 재시도)을
  그대로 따른다. 경로는 leg 열로 분할된다 (예: [차선 통과 구간] + [합류 구간]).
  존 하나씩 점진 획득하는 방식은 관제의 교착 방지 장치(오름차순+일괄)를 우회하므로
  쓰지 않는다. ※ leg 분할 기준은 관제 담당 팀원과 합의 필요 (인터페이스는 동일,
  호출 규약만의 문제).
- 내부 루프 (~50 Hz):
  1. pose 갱신 (odom 구독 최신값)
  2. 현재 웨이포인트 대비 오차(x, z, yaw) → P제어 → cmd_vel
     (기존 검증된 hold 보정 게인 재사용, `m3_drive_localize_test.py` 계열)
  3. **leg 게이트**: 다음 leg 경계 1.0 m 전에 그 leg의 존들을 일괄 acquire.
     허가 → 감속 없이 통과 / 거절 → 경계 0.5 m 앞 정지, `retry_after_sec` 간격 재시도
  4. leg를 완전히 벗어나면(로봇 중심이 경계+마진 통과) 그 leg의 존들 일괄 release
- 규칙: **다음 leg를 획득한 뒤에만 현재 leg를 release** (자기 위치는 항상 점유 중).
- 경로(웨이포인트+존 열+leg 분할)는 `lane_zones.py`가 `marker_layout.py`에서 파생:
  차선 2줄(z=±2.5), 존 = 마커 간격 3.40 m 세그먼트, leg = 차선 통과/합류 구간 묶음.
...
1. **관제의 요청 단위 규약** — leg 일괄 획득(오름차순 + 전부-아니면-무)이 한 요청
   안에서의 부분 보유 대기를 원천 차단한다 (관제 코드에 이미 구현된 장치).
2. **경로 배정 규칙** — leg 사이의 사이클("r1이 legA 쥐고 legB 대기, r2가 legB 쥐고
   legA 대기")은 요청 규약만으로 못 막는다. 2대 데모 규칙: **반대 방향으로 같은
   차선 leg를 공유하는 경로를 배정하지 않는다** (감지 시 후순위 로봇을 공유 구간
   진입 전에 대기). 기본 시나리오는 차선이 분리되어(robot1: z=+2.5 동진,
   robot2: z=−2.5 서진) 공유가 합류 leg 하나뿐이므로 사이클이 성립하지 않는다.

호출 규약(팀원 상정 = 경로 전체 일괄 vs 본 설계 = leg 단위 일괄)은 관제 담당
팀원과 합의 후 확정한다. 전체 일괄이면 교착은 더 단순해지지만 경로가 겹치는 순간
후발 로봇이 출발 자체를 못 하므로(동시 주행 데모 불성립), leg 단위를 제안한다.
N대 일반화는 parking_control 통합 후 과제.
...
- `parking_robot_interfaces` 패키지가 어느 브랜치에도 없음 — 존 서비스 타입을
  공유하려면 관제 담당 팀원에게 받아야 함. 확보 전에는 동형 srv를 임시 정의해
  진행하고, 원본 수령 시 교체(필드가 다르면 그 시점에 정합).
