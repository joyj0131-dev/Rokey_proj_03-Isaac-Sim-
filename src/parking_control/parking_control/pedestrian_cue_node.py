...
순간에 간단한 신호(/pedestrian_cue, geometry_msgs/Twist)를 쏜다.
...
메시지 타입이 왜 Twist인가(String이 아니라): Isaac Sim의 OmniGraph ROS2 브리지는
표준 std_msgs(String/Bool)용 전용 구독 노드가 없다(공식 문서 확인, 5.1.0 기준
Twist/Clock/JointState/TransformTree 등 특정 타입 전용 노드만 있고 나머지는
범용 "ROS2 Subscriber" 노드로 커스텀 설정이 더 필요함). 반면 Twist 구독
(`isaacsim.ros2.bridge.ROS2SubscribeTwist`)은 이 저장소에서 이미 실제로 쓰여
검증된 패턴이다(aruco_sim_bringup.py — "/cmd_vel에 한 번 pub하면 트리거 발동").
그래서 같은 확실한 방식을 재사용해 linear.x 값으로 신호를 구분한다:
  1.0 = entry_depart, 2.0 = exit_arrive, 0.0 = (신호 없음/유휴)
...
CUE_CODES = {"entry_depart": 1.0, "exit_arrive": 2.0}
...
class PedestrianCueNode(Node):
    ...
    def __init__(self):
        ...
        self._cue_pub = self.create_publisher(Twist, "pedestrian_cue", 10)
    ...
    def _publish_cue(self, cue: str, task_id: str) -> None:
        twist = Twist()
        twist.linear.x = CUE_CODES[cue]
        self._cue_pub.publish(twist)
