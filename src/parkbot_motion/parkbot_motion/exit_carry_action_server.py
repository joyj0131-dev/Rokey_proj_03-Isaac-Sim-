...
class CarryActionServer(Node):
    def __init__(self):
        ...
        self.declare_parameter('action_name', 'carry_to_slot')
        ...
        self.action_name = str(gp('action_name'))
        ...
        self._server = ActionServer(
            self, CarryToSlot, self.action_name, self._execute,
            callback_group=self._cbg,
            goal_callback=lambda _g: GoalResponse.ACCEPT,
            cancel_callback=lambda _g: CancelResponse.ACCEPT)
        self.get_logger().info(
            f'carry_action_server 시작: action={self.action_name} '
            '(월드프레임 반평행 중심제어)'
        )
