...
class NavigateActionServerNode(Node):
    ...
    def _on_navigate_to_pose(self, goal_handle):
        ...
        if mode == 'carry_bay':
            ...
            # 이 단계는 PICKED_UP(control_lift UP) 다음이라 차를 이미 강체로 든 상태다 —
            # 슬롯 축(예 90도)과 통로 축(0도)이 다르면 든 채 그대로 옮길 때 통로/개구부
            # 폭에 차가 대각선으로 걸리므로, 움직이기 전에 먼저 통로 축(0도)으로 되돌린다
            # (입차가 슬롯 앞에서 슬롯 축으로 미리 도는 것과 정반대 대칭). 이미 0도면
            # rotate_car_to_axis가 거의 즉시 끝나는 사실상 무동작이라 안전하다.
            ok = self.formation.rotate_car_to_axis(0.0)
            if ok:
            ...
            #   ① 슬롯 x열까지 통로(z=AISLE_Z)를 따라 동진(개구부 통과)
            #   ② 슬롯이 통로와 다른 축을 요구하면(slot_pose.orientation) 슬롯 앞에서 그
            #      축으로 피벗 회전(차를 든 채, rotate_car_to_axis — 위치 보정 포함)
            #   ③ 슬롯으로 진입.
            # (이미 통로에 있으면 ①은, 이미 정렬돼 있으면 ②는 사실상 즉시 끝난다.)
            yaw_map_rad = _yaw_from_quaternion(goal.pose.pose.orientation)
            target_axis_usd = math.radians(map_to_usd_yaw_deg(math.degrees(yaw_map_rad)))
            ok = self.formation.carry_to(tx_usd, AISLE_Z)
            if ok:
                ok = self.formation.rotate_car_to_axis(target_axis_usd)
            if ok:
