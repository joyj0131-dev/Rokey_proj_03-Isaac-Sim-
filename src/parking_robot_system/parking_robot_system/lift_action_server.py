...
DOWN -> SetBool(data=False)를 두 로봇의 /robot_{robot}/arm_control
...
# 기다려 설정된 vehicle_pose_topic의 Y 상승을 측정했는데, Task 10a 이식 때 이 대기가
# 빠졌다. 여기서 복원한다.
...
class LiftActionServerNode(Node):
    ...
    def __init__(self):
        ...
        vehicle_pose_topic = self.get_parameter("vehicle_pose_topic").value

        grp = ReentrantCallbackGroup()
        self.arm = {
            r: self.create_client(
                SetBool, f'/robot_{r}/arm_control', callback_group=grp)
    ...
    def _wait_lift_complete(self):
        """arm UP 지령 후 담당 차량 pose의 Y가 실제로 상승·안정될 때까지 대기.
    ...
    def _wait_settled(self, timeout=12.0, stable_for=1.5, eps=0.004, min_time=1.0):
        """DOWN 후 차량이 완전히 안착할 때까지(차량 Y가 stable_for초 동안 eps 이내로
    ...
    def _call_arms(self, opening):
        ...
        두 로봇의 /robot_<id>/arm_control(SetBool)에 opening을 비동기 호출.
    ...
    def _on_control_lift(self, goal_handle):
        ...
        if ok and opening:
            ...
            ok = self._wait_settled()   # ① 차량이 바닥에 완전히 내려앉을 때까지
            if ok:
                ok = self._wait(ARM_FOLD_TIME)  # ② 팔이 완전히 접혀 빠질 때까지
