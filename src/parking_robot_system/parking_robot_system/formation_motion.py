...
# --- rotate_car_to_axis 전용(dock_lift_handoff_mission.py L57-61/108 그대로, 실측 검증된 값) ---
# 부축(주 회전축 아닌 쪽) 위치 유지 보정 게인/상한. _carry_axis류와 동일한 방식으로,
# 회전 중 하중 쏠림(front_bias)에 의한 중심 이탈을 반대로 당겨 상쇄한다.
SIDE_K, SIDE_MAX = 0.8, 0.30
# 주의: formation_driver의 MAX_LIN(1.08)/MAX_YAW(0.36)은 "차를 안 든" 일반 주행/carry_to
# 용으로 1.2배 상향된 값이다(그 파일 주석: "원래 0.6"/"원래 0.15"). rotate_car_to_axis는
# 차를 "든 채" 회전시키는 용도라 원본이 의도적으로 더 보수적인 값을 썼다 — MAX_YAW=0.15는
# 회전 중 발진(진동)을 막으려고, MAX_LIN=0.6은 위치 보정 성분이 과하게 튀지 않도록 남긴
...
def axis_alignment_rotation(current_yaw, target_axis_rad):
    """직사각형 차량을 슬롯 축(mod pi, 코/꼬리 무관)에 맞추는 최소 회전량(rad, signed).

    dock_lift_handoff_mission._axis_alignment_rotation / parking_control/core/
    pivot_rotate_controller.axis_alignment_rotation과 동일 식(이 파일의 x,z 규약 기준)."""
...
class FormationMotion:
    ...
    def _veh(self, m):
        ...
        # 회전 판단은 로봇 각자 yaw가 아니라 차량 실측 yaw로 한다(그립에 슬립이 있어도
        # 차가 실제로 얼마나 돌았는지는 이 값이 정답 — dock_lift_handoff_mission._veh와 동일).
        self.veh_yaw = math.atan2(2 * (q.w * q.z + q.x * q.y), 1 - 2 * (q.y * q.y + q.z * q.z))
    ...
    def pickup_at_slot(self, slot_x, slot_z):
        ...
        self._settle(INGRESS_SETTLE)
        # 주의: 여기서는 아직 차를 "들지" 않았다(진입만 한 상태 — 파지/리프트는
        # orchestrator의 다음 단계인 control_lift('UP')에서 일어난다). rotate_car_to_axis는
        # 차를 강체로 문 상태를 전제하는 회전이라 여기서 돌리면 차는 안 돌고 로봇만
        # 빈 차체 밑에서 헛돌게 된다 — 그래서 통로 방향 재정렬은 여기서 하지 않고,
        # 리프트가 실제로 올라온 뒤(navigate_action_server의 'carry_bay' 단계 진입 시점)
        # 로 옮겼다(그쪽 참고).
    ...
    # ---- 신규(원본 dock_lift_handoff_mission._rotate_car_to_axis 이식) ----
    def rotate_car_to_axis(self, target_axis_rad, timeout=120.0):
        """차량을 든 채로 두 로봇이 차량 중심(rear/front 중점) 기준 반대 방향 피벗
        회전으로 target_axis_rad(mod pi, 코/꼬리 무관)에 맞춘다.

        carry_rotate_to(위)와의 차이: carry_rotate_to는 로봇 각자의 yaw를 절대각으로
        맞추는 "빈 로봇 정렬"용(EXIT pickup_at_slot이 차를 잡기 *전*에 씀 — 이때는
        차를 안 들었으니 하중 쏠림 드리프트가 없다). 이 메서드는 반대로 차를 "든 채"
        회전할 때 전용이다:
          - 판단 기준은 로봇 각자 yaw 평균이 아니라 /vehicle/pose 실측 veh_yaw(그립
            슬립 영향 없음).
          - 강체 운동학: 중심에서 오프셋 r인 점의 접선속도 = omega × r. 두 로봇이 중심을
            사이에 두고 반대편에 있어 부호가 식 하나로 자동 반대가 된다.
          - 차량 하중이 앞/뒤 한쪽에 쏠려 있으면(front_bias) 두 로봇에 크기가 같고
            방향만 반대인 명령을 내려도 실제로는 완전히 상쇄되지 않아 중심이 슬금슬금
            밀린다 — 회전 시작 시점 중심 좌표를 기억해두고 매 틱 벗어난 만큼 반대로
            당기는 위치 보정(hold_vx/hold_vz)을 접선속도에 더해 상쇄한다(SIDE_K/SIDE_MAX는
            _carry_axis류에서 이미 실측 검증된 값 재사용).
        """
        ...
        while time.time() < end:
            ...
            hold_vx = clamp(-SIDE_K * (cx - start_cx), SIDE_MAX)
            hold_vz = clamp(-SIDE_K * (cz - start_cz), SIDE_MAX)
            for rid in ROBOTS:
                x, z, yaw = self.pose[rid]
                rx, rz = x - cx, z - cz
                world_vx = omega * rz + hold_vx
                world_vz = -omega * rx + hold_vz
                fwd, left = body_twist_from_world_error(world_vx, world_vz, yaw)
                self._pub(rid, clamp(fwd, ROTATE_MAX_LIN), clamp(left, ROTATE_MAX_LIN), omega)
            time.sleep(1.0 / CONTROL_HZ)
        self._stop_all()
        if self.veh_yaw is None:
            return False
