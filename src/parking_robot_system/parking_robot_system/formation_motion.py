...
# 출차 하차(인계베이) 후 복귀 시 차 길이축(z)으로 빠져나올 거리. 인계베이 차량은 z=0 중심,
# 차체 반길이 ≈2.9라, 로봇이 완전히 벗어나려면 |z|>~3.4. 개구부(z∈[-4.5,4.5]) 안에 있어야
# 통과 가능하므로 3.5로 둔다(차 벗어남 + 개구부 통과 둘 다 만족).
BAY_CLEAR_Z = 3.5
...
class FormationMotion:
    ...
    def return_from_bay(self):
        """출차 하차(인계베이) 후 도크 복귀 — 차량 하부에서 '차 길이축(z)을 따라' 빠져나온다.

        인계베이 차량은 z=0 중심, 로봇은 차 밑 축 위치(rear z≈-1.9 / front z≈+1.7)에 있다.
        기존 return_both_to_docks처럼 동쪽(x, 차 폭 방향)으로 빼면 좌우 바퀴를 스친다 —
        진입은 z축을 따라 차 밑으로 들어갔으므로, 그 역방향(z축)으로 나와야 한다(사용자 요구).
        rear는 남(-z)/front는 북(+z)으로 각자 가까운 차 끝을 향해 빠져나온다(서로 반대 방향
        이라 겹치지 않음). 차를 벗어난 뒤(|z|=BAY_CLEAR_Z, 개구부 안) 그 z로 개구부를 지나
        동진하고, 각자 초기 도크(rear West_B / front West_A)로 간다.
        """
        routes = {}
        for rid, clear_z, dock_z in (("robot_rear", -BAY_CLEAR_Z, DOCK_Z_REAR),
                                     ("robot_front", BAY_CLEAR_Z, DOCK_Z_FRONT)):
            cur = self.pose.get(rid)
            if cur is None:
                return False
            cur_x = cur[0]
            # ① 차 길이축(z)으로 차 밖까지 → ② 그 z로 개구부 지나 동진 → ③ 도크로
            routes[rid] = [(cur_x, clear_z), (DOCK_X, clear_z), (DOCK_X, dock_z)]
        self.node.get_logger().info("출차 복귀: 차 길이축(z)으로 빠져나와 도크로")
        ok = self.approach_parallel(routes)
        self._stop_all()
        return ok
