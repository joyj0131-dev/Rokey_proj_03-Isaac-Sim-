...
두 세트로 나누는 이유(2026-07-24, 하드웨어 배치 확정): 로봇 4대가 입차 전용
(robot_rear/robot_front) / 출차 전용(robot_rear2/robot_front2)으로 고정 배치됐다.
navigate_action_server/align_action_server/lift_action_server는 FormationMotion을
통해 물리 로봇 이름 + 인계지점/게이트/도크/전용 차로 좌표를 직접 다루므로 세트별로
다른 파라미터 값을 줘 각자 다른 ROS 네임스페이스(entry/, exit/)에 띄운다.
robot_task_orchestrator/vehicle_detection_node는 좌표 대부분과 무관하지만, 액션 서버
이름(execute_parking_task, detect_vehicle)이 네임스페이스로 분리돼야 하므로 마찬가지로
세트마다 하나씩 띄운다. parking_control의 task_dispatcher_node가 request_type(ENTRY/
EXIT)에 따라 entry/execute_parking_task 또는 exit/execute_parking_task로 goal을 보낸다.

좌표는 전부 v3.usd 실측(Navigation/RobotServiceArea 스코프, 2026-07-24 확인) 기반이다
— generate_map.py의 SLOTS_USD/ENTRY_*_USD/EXIT_*_USD/DOCK_*_USD 상수와 같은 원천.
carry 구간(인계지점↔슬롯)에서 입차는 lane_z=-5.3, 출차는 lane_z=+5.3을 써서 두 세트가
물리적으로 다른 차로를 지나가게 한다 — 동시에 움직여도 서로 안 겹친다.
...
# 세트별 물리 상수 — v3.usd 실측(USD 프레임 x,z, FormationMotion과 동일 좌표계).
# bay_x_map/bay_y_map만 map 프레임(robot_task_orchestrator._bay_pose용, frame_transform
# 규약 x_map=x_usd/y_map=-z_usd로 변환된 값) — 출차 세트에서만 실제로 쓰인다.
SITES = {
    'entry': dict(
        rear_id='robot_rear', front_id='robot_front',
        handoff_x=-8.5, handoff_z=-5.5, gate_x=-13.0,
        dock_rear_x=-3.2, dock_rear_z=-2.2, dock_front_x=-1.2, dock_front_z=-2.2,
        lane_z=-5.3, site_role='entry',
        pickup_x_usd=-8.5, pickup_z_usd=-5.5,
        bay_x_map=-8.5, bay_y_map=5.5,   # 이 세트에서는 안 쓰이지만 무해하게 채워둠
    ),
    'exit': dict(
        rear_id='robot_rear2', front_id='robot_front2',
        handoff_x=-8.5, handoff_z=5.5, gate_x=-13.0,
        dock_rear_x=-3.2, dock_rear_z=2.2, dock_front_x=-1.2, dock_front_z=2.2,
        lane_z=5.3, site_role='exit',
        pickup_x_usd=-8.5, pickup_z_usd=5.5,   # 이 세트에서는 안 쓰임(detect_vehicle 미사용)
        bay_x_map=-8.5, bay_y_map=-5.5,
    ),
}
