...
이를 예측(휠오도 `predict_body`)·보정(마커 `update`)과 묶어 폐루프 주행으로 감싼다. 회전은
`mecanum_drive`의 회전 역기구학을 실측 재보정(`SIGN_YAW=-1.0`, `YAW_SCALE=0.4080`)했고 오도
잔차 보정만 `YAW_ODOM_SCALE=1.1677`로 남긴다(경위는 아래 "회전 기구학" 항), 도크체크·XN정렬 보정은
...
entry_follow **~1.3~1.4cm**, entry_lead(+x 충돌회피 오프셋 포함) **~15~18cm**.
양 로봇 **GT 실측 분리거리 1.60~1.64m**(4개 판독 전부 요구치 ≥1.5m 충족, `MISSIONB_SEPARATION`
토큰으로 매 실행 출력).

**회전 기구학(2026-07-25 재보정 — 중요)**: 사용자가 GUI에서 "90°가 아니라 270°를 돈다"고 관찰한
것을 계기로 `--probe=YAWSTEP`(개루프·고정시간·unwrap GT)으로 실측한 결과, 명령 대비 실제 회전비가
**k=−2.7448**(부호 반대·2.7배)였다. `mecanum_drive`의 `SIGN_YAW`·`YAW_SCALE`이 실제 물리와
달랐던 것으로, 90°·180°는 **mod 360으로 오차가 완전히 숨어** 종단각만 보던 검증(`ROTCHK`,
미션 `err_yaw_gt`)이 전부 통과했다. 재보정 후 **k=+1.1009**, 45° 회전 오차 **1.93°**(이전 설계라면
≈−135°로 파탄), 90° 명령의 실제 스윕 경로 ≈95°. `YAW_ODOM_SCALE`은 1.0으로 내리면 미션이 깨져
(실측 `ok=False`, 오차 0.54~0.64m) 롤러 슬립 잔차분 **1.1677**을 유지한다.

**정직한 한계**: (1) 위치전용 보정의 대가로 최종 헤딩 정확도는 마커가 아니라 오도메트리 한계에
묶여 있다(현재 `ROTCHK` 90° 기준 ~5.5°). (2) entry_lead의 +x 오프셋 구간은 후반부에 XN이
카메라 시야를 벗어나 순수 오도로 주행한다 — 종단오차 15~18cm는 헤딩 잔차 5.45° × 오프셋 1.7m
(=1.7·sin5.45°≈16cm)로 설명되며, 오차 방향이 분리축(x)과 수직이라 위 GT 분리거리에는 거의
영향하지 않는다. (3) 충돌 클리어런스의 로봇 풋프린트(반경 0.68m 원)는 여전히 실측 미검증이다.
