...
class Ros2DualDataSource(DataSource):
    ...
    def create_request(self, payload) -> ParkingRequest:
        ...
        # DB 라우팅 표는 UI 모델명(PARK_IN/PARK_OUT)을 저장하고, 실제 ROS
        # RequestParkingTask 서비스에는 ENTRY/EXIT를 보낸다. 두 값을 섞으면
        # PARK_OUT 행을 EXIT 키로 조회해 출차 그룹을 찾지 못한다.
        ui_request_type = payload.request_type.value
        ros_type = _REQUEST_TYPE_TO_ROS[payload.request_type]
        group = self._groups_by_request_type.get(ui_request_type)
        if group is None:
            raise DataSourceError(
                f"{ui_request_type} 요청을 처리할 로봇 그룹이 라우팅 표(DB)에 없습니다.",
                status_code=500,
            )
        ...
        if client is None or not client.wait_for_service(
            ...
            db.log_dispatch(
                task_id=None, group_id=group_id, request_type=ui_request_type,
        ...
        def _on_done(future) -> None:
            ...
            outcome["response"] = None if exception is not None else future.result()
            done.set()
        ...
        if not response.accepted:
            ...
            db.log_dispatch(
                task_id=response.task_id or None, group_id=group_id,
                request_type=ui_request_type,
                vehicle_number=vehicle_number, accepted=False, message=message,
            )
            raise DataSourceError(message, status_code=409)

        db.log_dispatch(
            task_id=response.task_id, group_id=group_id,
            request_type=ui_request_type,
