...
INGRESS_SPEED = 0.30   # 원래 검증됐던 값으로 되돌림(0.45로 시연용 상향했다가 복귀).
                        # 차량이 삐딱해지는 문제가 속도 탓인지 분리 확인하려는 목적 —
                        # dock_motion_check.py가 PASS 났던 원래 값.
CARRY_SPEED = 0.30     # 통로 횡단(x축)용. 마찬가지로 원래 검증값(0.60→0.30)으로 복귀.
SLOT_ENTRY_SPEED = 0.30   # 슬롯 진입(z축)용. carry_speed와 같이 원래 검증값으로 복귀.
...
# yaw 오차가 클수록 주축(전진) 속도를 낮춘다 — wz 보정 상한은 전진 속도와 무관하게
# 고정인데, 교란(하중 비대칭 토크)은 전진 속도에 비례해 커질 걸로 보여서, 많이
# 틀어졌을 때 전진을 늦추면 교란은 줄고 보정 힘은 그대로라 더 잘 수렴한다.
YAW_SLOW_1 = math.radians(1.0)    # 이 이하면 정상 속도
YAW_SLOW_2 = math.radians(3.0)    # 이 이상이면 최소 배율까지 감속
YAW_SLOW_MIN_SCALE = 0.3          # 완전 정지는 안 함(타임아웃/스톨 방지)
...
class HandoffMission(Node):
    ...
    def _carry_axis(self, mode, target, hold, speed, hold_yaw=FACE_MZ,
        ...
        주축 속도는 yaw 오차 크기에 따라 스케일다운한다(YAW_SLOW_1~2) — wz 보정
        상한(CARRY_WZ_MAX)은 전진 속도와 무관하게 고정인데, 교란(하중 비대칭
        토크)은 전진 속도에 비례해 커질 것으로 보여서, 많이 틀어졌을 때 전진을
        늦추면 "교란은 줄고 보정 힘은 그대로"라 더 잘 수렴한다."""
        end = time.time() + timeout
        ok = False
        last_log = time.time()
        while time.time() < end:
            ...
            # yaw 오차가 크면 주축 속도를 낮춘다 — wz 보정 상한(CARRY_WZ_MAX)은 전진
            # 속도와 무관하게 고정인데, 교란(하중 비대칭 토크)은 전진 속도에 비례해
            # 커질 걸로 보여서, 많이 틀어졌을 때 전진을 늦추면 교란은 줄고 보정 힘은
            # 그대로라 더 잘 수렴한다.
            abs_eyaw = abs(eyaw)
            if abs_eyaw <= YAW_SLOW_1:
                speed_scale = 1.0
            elif abs_eyaw >= YAW_SLOW_2:
                speed_scale = YAW_SLOW_MIN_SCALE
            else:
                frac = (abs_eyaw - YAW_SLOW_1) / (YAW_SLOW_2 - YAW_SLOW_1)
                speed_scale = 1.0 - frac * (1.0 - YAW_SLOW_MIN_SCALE)
            scaled_speed = speed * speed_scale
            # world 프레임에서 곧바로 부호를 정한 뒤(err>0 이면 그 축의 +방향으로),
            # 실측 veh_yaw로 body(fwd/left)로 변환한다(_omni_step과 같은 식) — 고정
            # 방위(FACE_MZ) 가정이 아니라 실제 방위 기준이라 hold_yaw가 뭐든 그
            # 방향으로 "전진"하게 된다.
            axis_w = scaled_speed if err > 0 else -scaled_speed
            ...
            if time.time() - last_log >= 1.0:
                self.get_logger().info(
                    f"슬롯 이동({mode}): 남은거리={abs(err):.2f}m 부축편차={side - hold:+.2f}m "
                    f"yaw오차={math.degrees(eyaw):+.1f}도 속도배율={speed_scale:.2f} "
                    f"fwd={fwd:+.2f} left={left:+.2f} wz={wz:+.3f}")
                last_log = time.time()
            for r in ROBOTS:
