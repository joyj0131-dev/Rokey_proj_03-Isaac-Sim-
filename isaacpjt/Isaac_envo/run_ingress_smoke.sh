#!/bin/bash
# R5a ingress_node 스모크 런처. run_lift_smoke.sh 와 동일하게 parking_robot_interfaces
# 빌드 결과(install/)를 PYTHONPATH 에 얹어야 IngressUnderTruck 액션 타입을 임포트할 수 있다.
...
exec python3 "$SCRIPT_DIR/ingress_smoke.py" "$@"
