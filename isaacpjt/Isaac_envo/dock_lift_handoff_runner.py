...
ROS2: /robot_N/cmd_vel 구독, /robot_N/odom 발행(x,y=높이,z,yaw),
      ...
      /sim_checkpoint_docked(Trigger): 로봇을 차 밑(축 정렬)으로 바로 순간이동
        — 접근·진입 구간 없이 파지 이후만 반복 테스트할 때 씀
        (mission의 /dock_lift_from_docked 와 짝).
...
def main():
    ...
    try:
        ...
        def apply_arms(key):
            ...
            if abs(tgt - cur) > 1e-4:
                cur += max(-0.035, min(0.035, tgt - cur))
                arm_applied[key] = cur
        ...
        FACE_MZ_YAW = math.pi / 2  # mission.py FACE_MZ 와 동일(차 밑 진입 방위).
        ROBOT_Y = 0.06             # _place_robot_dock 초기 높이와 동일.

        def _yaw_quat(yaw):
            return np.array([[math.cos(yaw * 0.5), 0.0, math.sin(yaw * 0.5), 0.0]],
                            dtype=np.float32)

        def _teleport(prim, pos_xyz, quat_wxyz):
            p = np.array([pos_xyz], dtype=np.float32)
            prim.set_world_poses(p, quat_wxyz)
            try:
                prim.set_velocities(np.zeros((1, 6), dtype=np.float32))
        ...
        def _on_checkpoint_docked(req, resp):
            """로봇을 차 밑(축 정렬 위치)으로 바로 순간이동 — 접근·진입 구간을 건너뛰고
            파지·운반 구간만 반복 테스트할 때 씀. 파지는 여전히 실제로(그립 클라이언트가)
            수행해야 한다(arm_cmd는 접힘 상태로 리셋)."""
            if not AXLE:
                resp.success = False; resp.message = "AXLE 미계산"; return resp
            targets = {"rear": AXLE["rear_z"], "front": AXLE["front_z"]}
            quat = _yaw_quat(FACE_MZ_YAW)
            for key, art in arts.items():
                _teleport(art, (AXLE["center_x"], ROBOT_Y, targets[key]), quat)
                arm_cmd[key] = 0.0
                arm_applied[key] = 0.0
            pos, orn = VEH_SNAPSHOT
            veh_rb.set_world_poses(np.array(pos, dtype=np.float32),
            ...
            try:
                ...
                node.get_logger().warn(f"속도 초기화 실패(무시): {e}")
            resp.success = True
            resp.message = "체크포인트 완료(차 밑 정렬) — /dock_lift_from_docked 로 이어서"
            node.get_logger().info(resp.message)
            return resp

        node.create_service(Trigger, "/sim_reset", _on_reset)
        node.create_service(Trigger, "/sim_checkpoint_docked", _on_checkpoint_docked)
