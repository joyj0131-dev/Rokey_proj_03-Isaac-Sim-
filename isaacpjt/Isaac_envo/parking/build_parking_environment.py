...
# USD에 구울 때는 반드시 상대경로를 쓴다. 절대경로로 구우면 이 트리를 옮기는 순간
# 모든 참조가 끊긴다(실제로 /home/rokey/Isaac_envo 시절 경로가 굳는 바람에 차량
...
def _vehicle_instance(
    ...
    vehicle.GetReferences().AddReference(
        VEHICLE_REF, f"/World/Vehicles/{vehicle_type}"
    )
...
def build_stage():
    ...
    scene.GetPrim().GetReferences().AddReference(
        VEHICLE_REF, "/World/PhysicsScene"
    )
    ...
    vehicle_physics.GetReferences().AddReference(
        VEHICLE_REF, "/World/VehiclePhysics"
    )
    ...
    fab_colors.GetReferences().AddReference(VEHICLE_REF, "/World/Looks/FabColors")
    colors = {
