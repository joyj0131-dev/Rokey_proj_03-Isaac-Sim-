#!/bin/bash
# R5a ingress_node 런처 (터미널 B, 시스템 ROS 2 Humble).
# Isaac 러너(--bridge)와 같은 도메인/화이트리스트를 쓴다. run_lift_action_server.sh
# 와 동일 패턴 — IngressUnderTruck 액션 타입(parking_robot_interfaces)은 코드생성이
# 필요해 콜콘 빌드 결과(install/)를 PYTHONPATH 에 얹어야 한다.
...
exec python3 -m parkbot_motion.ingress_node "$@"
