#!/usr/bin/env python3
"""특정 스텝에서 뎁스 프레임의 행(row)별 프로파일(중앙 40% 열의 최소/평균)을 출력해
ROI(세로 하단 50%)가 실제로 어떤 신호를 보는지 정밀 확인한다."""
...
CAM_RES = (640, 480)
CAPTURE_STEPS = set(range(1, 1201, 30))
TELEPORT_BACK_Z = 1.0  # 실제 트럭 뒤 훨씬 뒤에서 출발시켜 접근 전체 과정을 관찰
    ...
    try:
        ...
        for step in range(1, 1201):
            ...
            if step in CAPTURE_STEPS:
                ...
                h, w = depth.shape
                col_lo, col_hi = int(w * 0.30), int(w * 0.70)
                strip = depth[:, col_lo:col_hi]
                z = rigid_position(ROBOT_ROOT)[2]
                print(f"--- step={step} robot_z={z:.3f} depth shape={depth.shape} ---", flush=True)
                for r0 in range(0, h, 80):
                    r1 = min(r0 + 80, h)
                    band = strip[r0:r1]
                    finite = band[np.isfinite(band)]
                    if finite.size:
                        print(f"  rows {r0:3d}-{r1:3d}: min={finite.min():.3f} mean={finite.mean():.3f} max={finite.max():.3f}", flush=True)
                    else:
                        print(f"  rows {r0:3d}-{r1:3d}: (no finite)", flush=True)
