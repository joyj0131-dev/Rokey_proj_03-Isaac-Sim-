...
```bash
export ROS_DOMAIN_ID=126                      # 팀에 맞게 수정
...
```
자동으로 Play되고 `/cmd_vel`을 구독한다. 콘솔에 `ROS2_DRIVE_READY topic=/cmd_vel domain=126 ...`이
...
```bash
export ROS_DOMAIN_ID=126                      # Isaac과 동일
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
