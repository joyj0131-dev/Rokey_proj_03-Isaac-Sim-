#!/bin/bash
# R4 lift_action_server 런처 (터미널 B, 시스템 ROS 2 Humble).
# Isaac 러너(--bridge)와 같은 도메인/화이트리스트를 쓴다. run_pose_controller_node.sh
# 와 동일 패턴이나, ControlLift 액션 타입(parking_robot_interfaces)은 코드생성이
# 필요해 콜콘 빌드 결과(install/)를 PYTHONPATH 에 얹어야 한다(순수 PKG_SRC 만으로는
# 임포트 안 됨 — src/parkbot_motion 의 다른 노드들과 다른 점).
...
exec python3 -m parkbot_motion.lift_action_server "$@"
