...
REL="${ISAAC_SIM_ROOT:-$HOME/dev_ws/isaac_sim/isaacsim/_build/linux-x86_64/release}"
...
if [[ ! -x "$REL/python.sh" ]]; then
  echo "Isaac Sim python.sh를 찾을 수 없습니다: $REL/python.sh" >&2
  echo "설치 위치가 다르면 ISAAC_SIM_ROOT를 지정하세요." >&2
  exit 2
fi
